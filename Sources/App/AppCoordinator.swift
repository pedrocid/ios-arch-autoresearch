...
open class AppCoordinator: @unchecked Sendable {
    public let profileVM: UserProfileViewModel?; public let productVM: ProductListViewModel; public let orderVM: OrderViewModel
...
extension User { public static func fetch(id: String) async throws -> User { try JSONDecoder().decode(User.self, from: try await APIClient.default.fetchUser(id: id)) } }
extension Product { public static func fetchAll(category: String) async throws -> [Product] { try JSONDecoder().decode([Product].self, from: try await APIClient.default.fetchProducts(category: category)) }; public func loadImage() async throws -> Data { guard let url = imageURL else { return ImageLoader.default.placeholderImageData() }; return try await ImageLoader.default.loadImage(from: url) } }
