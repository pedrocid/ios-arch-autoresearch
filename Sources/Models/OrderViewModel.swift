...
open class OrderViewModel: @unchecked Sendable {
    ...
    public init() {}
    public func createOrder(userId: String, products: [Product]) -> Order { let order = Order(id: UUID().uuidString, userId: userId, products: products); orders.append(order); return order }
...
public struct UserProfileViewModel: Sendable {
    ...
    public init(userId: String, displayName: String, email: String, avatarData: Data? = nil, isActive: Bool = true) {
        self.userId = userId; self.displayName = displayName; self.email = email; self.avatarData = avatarData; self.isActive = isActive
    }
}
