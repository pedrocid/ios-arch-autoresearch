...
open class StorageManager: @unchecked Sendable {
    ...
    public func formattedSummary() -> String { "Storage contains \(inMemoryStore.count) items: [\(inMemoryStore.keys.sorted().joined(separator: ", "))]. Operations: \(operationCount)" }
    private func logOperation(_ op: String, key: String) { lastError = nil; print("[\(ISO8601DateFormatter().string(from: Date()))] StorageManager.\(op): \(key)") }
}
