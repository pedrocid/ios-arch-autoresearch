...
open class StorageManager: @unchecked Sendable {
    ...
    public func save(key: String, value: Any) {
        inMemoryStore[key] = value; operationCount += 1; logOperation("SAVE", key: key)
    }
    public func load(key: String) -> Any? {
        operationCount += 1; logOperation("LOAD", key: key); return inMemoryStore[key]
    }
    public func delete(key: String) {
        inMemoryStore.removeValue(forKey: key); operationCount += 1; logOperation("DELETE", key: key)
    }
    public func clear() { inMemoryStore.removeAll(); operationCount = 0 }
    ...
    private func logOperation(_ op: String, key: String) {
        lastError = nil; print("[\(ISO8601DateFormatter().string(from: Date()))] StorageManager.\(op): \(key)")
    }
}
